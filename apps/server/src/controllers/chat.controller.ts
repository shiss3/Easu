...
- 回复格式范例：
根据您【用户输入的标签，如果输入了具体时间则回答用户满足您的时间安排即可】的需求，小宿为您找到了一些满足您的需求，并且风评好的酒店：
--例子：根据您在上海的入住时间安排和近地铁的设施/服务需求，小宿为您精选了符合您要求的酒店：
...
例子：价格：526起
地点：【酒店地址】
例子：朝阳区某某路508号，距地铁站250米
推荐理由：该酒店满足您的【用户输入的标签】需求，距离地铁近，带免费早餐。
-例子：根据您在上海的入住时间安排和近地铁的设施/服务需求，小宿为您精选了符合您要求的酒店：
    ...
    function: {
        ...
        parameters: {
            ...
            properties: {
                ...
                checkIn: {
                    type: 'string',
                    description: '入住日期，格式 YYYY-MM-DD。从用户对话中推算，如果用户没有提及没有默认为今天。',
                },
                checkOut: {
                    type: 'string',
                    description: '离店日期，格式 YYYY-MM-DD。从用户对话中推算，如果没有没有提及没有默认为明天。',
                },
                keyword: {
                    type: 'string',
                ...
                minPrice: {
                    type: 'number',
                    description: '最低价格（元）。【极度重要】：仅在用户明确提到预算下限时传入。千万不要自行猜测或使用默认值。',
                },
                maxPrice: {
                    type: 'number',
