...
Easu 是一个面向 C 端用户的智能酒店预订平台，核心定位为：**结合大语言模型（LLM）能力的现代化移动端酒店预订应用**。用户可以通过传统的表单筛选搜索酒店，也可以通过自然语言与 AI 助手"小宿"对话，获得个性化的酒店推荐。

本人负责 **client-user 用户端** 的前后端全栈开发，涵盖从数据库设计、RESTful API 实现到前端 UI 交互的完整链路。

### 1.1 技术选型

| 层级 | 技术栈 | 说明 |
|------|--------|------|
| **前端框架** | React 19 + TypeScript | 使用最新 React 版本，全量 TypeScript 开发 |
| **构建工具** | Vite 7 | 极速 HMR，生产环境 Tree-shaking |
| **路由** | React Router DOM v7 | SPA 路由管理 |
| **状态管理** | Zustand | 轻量级状态管理，用于搜索条件、用户信息、AI 会话等全局状态 |
| **数据请求** | React Query (TanStack Query) | 服务端状态缓存、自动重试、无限滚动分页 |
| **样式方案** | TailwindCSS 3 | 原子化 CSS，移动端优先响应式设计 |
| **UI 组件** | shadcn/ui + Lucide Icons | 基于 Radix UI 的无头组件 + 精美图标库 |
| **后端框架** | Express 5 + TypeScript | Node.js HTTP 服务 |
| **ORM** | Prisma 7 | 类型安全的数据库 ORM，自动生成 TypeScript 类型 |
| **数据库** | PostgreSQL | 关系型数据库，支持 GIN 索引与 `pg_trgm` 模糊匹配 |
| **AI 能力** | OpenAI SDK（接入豆包大模型） | 通过 Function Calling 实现 AI Agent 工作流 |
| **实时通信** | Server-Sent Events (SSE) | AI 对话流式输出 |
| **安全** | JWT 双 Token + Helmet + Rate Limit + Zod + XSS | 完整的安全防护体系 |
...
│
├── apps/                      # 应用层
│   ├── client-user/           # 📱 C端用户前端（本人负责）
│   ├── client-merchant/       # 💼 B端商家管理前端（其他成员负责）
│   └── server/                # 🖥️ 后端服务（本人负责用户端相关 API）
│
└── packages/                  # 共享包层
    ├── database/              # 🗄️ Prisma Schema + 数据库客户端
    │   ├── prisma/
...
| 成员 | 职责范围 |
|------|----------|
| 本人 | client-user 用户端前端 + server 后端（用户端相关 API）+ 共享包维护 |
| 成员 B | client-merchant 商家端前端 + server 后端（商家端相关 API） |

通过 Monorepo + 共享包的架构，团队成员可以并行开发各自负责的模块，同时共享数据库模型和工具函数，极大提高了协作效率。
...
本项目通过 Monorepo 架构实现了高效的团队协作，通过 AI Agent + Function Calling 的深度集成为传统酒店预订场景带来了全新的交互体验。在技术实现上，从 SSE 流式通信、三级降级搜索引擎、JWT 双 Token 无感刷新到节假日日历等，每个模块都经过了精心设计和实战打磨。

项目的核心价值不仅在于功能的完整性，更在于 **AI 能力与业务场景的有机融合**——用户不需要学习复杂的搜索表单，只需像和朋友聊天一样说出需求，AI 助手就能理解意图、搜索酒店、生成推荐，真正实现了"让旅程有处可栖"。
